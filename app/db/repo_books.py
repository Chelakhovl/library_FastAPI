...
async def create_book(session: AsyncSession, *, title: str, author: str, genre: str, published_year: int) -> dict:
    q = text("""
        SELECT author_get_or_create(:author) AS author_id
    """)
    ...
    """)
    row = (await session.execute(q2, {
        "title": title, "author_id": author_id, "genre": genre, "year": published_year, "author": author
    })).mappings().one()
...
async def update_book(session: AsyncSession, book_id: int, *, title: Optional[str], author: Optional[str],
                      genre: Optional[str], published_year: Optional[int]) -> Optional[dict]:
    author_id = None
    if author is not None:
        res = await session.execute(text("SELECT author_get_or_create(:a) AS author_id"), {"a": author})
        author_id = res.scalar_one()
    ...
    if author_id is not None:
        sets.append("author_id = :author_id"); params["author_id"] = author_id
    if genre is not None:
        sets.append("genre = :genre"); params["genre"] = genre
    if published_year is not None:
        sets.append("published_year = :year"); params["year"] = published_year
