...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "author_id",
            sa.BigInteger,
            sa.ForeignKey("public.authors.id", ondelete="RESTRICT"),
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            "genre IN ('Fiction','Non-Fiction','Science','History')",
            name="books_genre_check",
        ),
        sa.CheckConstraint(
            "published_year BETWEEN 1800 AND EXTRACT(YEAR FROM CURRENT_DATE)::INT",
            name="books_year_check",
        ),
        schema="public",
    )
    ...
    # Індекси
    op.create_index(
        "idx_books_lower_title", "books", [sa.text("lower(title)")], schema="public"
    )
    op.create_index(
        "idx_authors_lower_name", "authors", [sa.text("lower(name)")], schema="public"
    )
    op.create_index(
        "idx_books_published_year", "books", ["published_year"], schema="public"
    )
